...
Example: 50 bins splits the 0-2 second range into 50 equal intervals.""",
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Active Time Bins in Piece 1", dstart_metrics["target_dstarts"])
        with col2:
            st.metric("Active Time Bins in Piece 2", dstart_metrics["generated_dstarts"])

        # Start time distributions
        st.subheader("Dstart Time Distributions")
        col1, col2 = st.columns(2)
        with col1:
            fig1 = plot_dstart_distribution(
                distribution=dstart_metrics["target_distribution"],
                n_bins=n_bins,
                title="Start Time Distribution 1",
            )
            st.plotly_chart(fig1)
        with col2:
            fig2 = plot_dstart_distribution(
                distribution=dstart_metrics["generated_distribution"],
                n_bins=n_bins,
                title="Start Time Distribution 2",
            )
        ...
        fig3 = plot_dstart_correlation_heatmap(
            target_dist=dstart_metrics["target_distribution"],
            generated_dist=dstart_metrics["generated_distribution"],
            n_bins=n_bins,
        )
